...
public class Runner extends Mob{
 ...
 public Runner(Board parenti, int x, int y){
  super(parenti, x, y);
  position[0]=x;
  position[1]=y;
  origin[0]=x;
  origin[1]=y;
  gold = parent.stats.runnermaxgold;
  parent=parenti;
  type=10;
  name="Runner";
  timer = new Timer(parent.stats.runnerspeed,this);
  timer.start();
 }
 ...
 public void interact(){
  ...
  if (parent.player.position[0]==target[0] && parent.player.position[1]==target[1]){
   parent.player.gold=parent.player.gold-parent.stats.runnerdamage;
  }
  for (int i=0; i<parent.allies.size(); i++){
   if (parent.allies.get(i).position[0]==target[0] && parent.allies.get(i).position[1]==target[1]){
    parent.allies.get(i).gold=parent.allies.get(i).gold-parent.stats.runnerdamage;
   }
  }
 }
 ...
 public void regenerate(){
  regeneration++;
  if (regeneration>=parent.stats.runnerregenerate){
   regeneration=0;
   if (gold<parent.stats.runnermaxgold){
    gold++;
   }
  }
 }
